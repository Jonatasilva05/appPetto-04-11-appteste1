  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // A API agora retorna 'token' e 'role'.
        // A função signIn do AuthContext vai armazenar ambos.
        if (data.token && data.role) {
            await signIn(data.token, data.role);
            
            // Agora, decidimos para onde ir com base na 'role'
            if (data.role === 'veterinario') {
                router.replace('/(vet)/dashboard'); // Rota para o dashboard do VET
            } else {
                router.replace('/(auth)/login'); // Rota padrão para o TUTOR
            }
        } else {
        ...
        >
          ...
          <View style={styles.signupContainer}>
            <TouchableOpacity onPress={() => router.push('/(auth)/cadastro')}>
                ...
                É um tutor? <Text style={styles.linkText}>Registre-se</Text>
                </Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => router.push('/(auth)/login')}>
                <Text style={styles.signUpText}>
                É um veterinário? <Text style={styles.linkText}>Cadastre-se aqui</Text>
                </Text>
            </TouchableOpacity>
          </View>
